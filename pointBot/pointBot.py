...
class pointBot(irc.IRCClient):
    ...
    # Compiled regex
    cmdExpr = compile(r"^([a-zA-Z]+)\b\s?((\w+)\s?(-?\d+)?$|.*)?")
    ptExpr = compile(r"^(\+|-)(\d+)\s(?:to\s)?(\w+).*$")
    timeExpr = compile(r"^(\d+):(\d+):(\d+)")
    ...
    def signedOn(self):
        # Join the channel, restore save files, grab user list
    ...
    # Called when a user leaves the IRC network. Marks player as inactive.
    def userQuit(self, user, quitMessage):
        index = self.getUserIndex(user)
        if (index != -1):
            self.userList[index].active = False
        return
    ...
    # Callback for the /who command, retrieves a list of users; used when we join/a new user joins the channel
    ...
    def adminCommands(self, nick, message):
        ...
        if not cmdMatch:
            ...
            rawMsg = cmdMatch.group(2)
            target = cmdMatch.group(3)
            value = cmdMatch.group(4)
        ...
        if command == "start":
            ...
            self.restore()
        elif command == "say" and rawMsg:
            self.msg(self.channel, rawMsg)
        elif command == "me" and rawMsg:
            self.describe(self.channel, rawMsg)
        # The one command that doesn't fit in to our parameters well; don't want to refactor regex more
        elif command == "msg" and rawMsg:
            self.msg(rawMsg.split(" ")[0], rawMsg.split(" ", 1)[1])
    ...
    # (Admin command) Restores user/points list, ignore list, and bot list from files, then grab names/activity status; called when we first join channel
    def restore(self):
        try:
            self.userList = []
        ...
        # Has irc_RPL_WHOREPLY() as callback
    ...
    def displayPoints(self):
        self.msg(self.channel, "Here is a list of points in the format 'User: Total Points' (* = active)")
        self.sortUserList()
        pointsList = ""
        for user in self.userList:
            statusChar = '*' if user.active else ''
            if user.ip not in self.ignoreList:
                pointsList += "|{}{}|: {}, ".format(user.nick, statusChar, user.totalPoints)
        self.msg(self.channel, pointsList[:-2])
        return
    ...
    def pointMessage(self, sender, sign, number, target):        
        ...
        if int(number) == 0 or self.userList[senderIndex].nick == target:
            self.msg(sender, "Nice try, dingus")
            return
        if (targetIndex == -1) or (self.userList[targetIndex].nick in self.ignoreList) or (not self.userList[targetIndex].active):
